...
class BenchStats {
  ...
  void report() {
    ...
    buf.writeln(
        '│  Ops/sec ${opsPerSec.toStringAsFixed(0).padLeft(12)}');
    buf.writeln('└─────────────────────────────────────────────────────');
    // ignore: avoid_print
    print(buf);
  }
...
void main() {
  group('🔥 WaffleRecord Creation Benchmark', () {
    test('Float32List constructor — 128d', () {
      final stats = runBench(
        'WaffleRecord(Float32List) dim=128',
        () {
          WaffleRecord(
            id: 'bench-id',
    ...
    test('Float32List constructor — 384d (embedding size)', () {
      final stats = runBench(
        'WaffleRecord(Float32List) dim=384',
        () {
          WaffleRecord(
            id: 'bench-id-384',
    ...
    test('Float32List constructor — 1536d (OpenAI size)', () {
      final stats = runBench(
        'WaffleRecord(Float32List) dim=1536',
        () {
          WaffleRecord(
            id: 'bench-id-1536',
    ...
    test('fromList factory — 1536d', () {
      ...
      final stats = runBench(
        'WaffleRecord.fromList() dim=1536',
        () {
  ...
  group('🔥 Vector Operations Benchmark', () {
    ...
    test('Dart-side cosine similarity — 128d', () {
      ...
      final stats = runBench(
        'Dart cosine similarity dim=128',
        () {
          _dartCosineSim(a, b);
        },
        iterations: 10000,
    ...
    test('Dart-side cosine similarity — 1536d', () {
      ...
      final stats = runBench(
        'Dart cosine similarity dim=1536',
        () {
          _dartCosineSim(a, b);
        },
        iterations: 5000,
    ...
    test('Vector flat-packing for batch insert — 100×128d', () {
      final vecs =
          List.generate(100, (i) => Float32List.fromList(
              List<double>.generate(128, (j) => (i + j) * 0.01)));

      final stats = runBench(
        'Flatten 100×128d vectors',
        () {
          final flat = <double>[];
          for (final v in vecs) {
            flat.addAll(v);
    ...
    test('Vector flat-packing for batch insert — 1000×128d', () {
      final vecs =
          List.generate(1000, (i) => Float32List.fromList(
              List<double>.generate(128, (j) => (i + j) * 0.01)));

      final stats = runBench(
        'Flatten 1000×128d vectors',
        () {
          final flat = <double>[];
          for (final v in vecs) {
            flat.addAll(v);
  ...
  group('🔥 Batch Record Preparation Benchmark', () {
    test('prepare 100 WaffleRecords for batch insert', () {
      final stats = runBench(
        'Build 100 WaffleRecords (dim=128)',
        () {
          List.generate(
            100,
            (i) => WaffleRecord(
    ...
    test('prepare 1000 WaffleRecords for batch insert', () {
      final stats = runBench(
        'Build 1000 WaffleRecords (dim=128)',
        () {
          List.generate(
            1000,
            (i) => WaffleRecord(
    ...
    test('prepare 10000 WaffleRecords for batch insert', () {
      final stats = runBench(
        'Build 10000 WaffleRecords (dim=128)',
        () {
          List.generate(
            10000,
            (i) => WaffleRecord(
  ...
  group('🔥 WaffleQueryResult Processing Benchmark', () {
    ...
    test('filter 1000 results by threshold', () {
      ...
      final stats = runBench(
        'Filter 1000 results (threshold=0.5)',
        () {
    ...
    test('strip metadata from 100 results', () {
      ...
      final stats = runBench(
        'Strip metadata from 100 results',
        () {
          results
              .map((r) => WaffleQueryResult(
                    id: r.id,
                    distance: r.distance,
                    metadata: null,
                  ))
